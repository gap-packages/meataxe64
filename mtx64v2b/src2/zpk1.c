...
int main(int argc,  char **argv)
{
    ...
    if(hdr[0]==1)
    {
printf(" is flat matrix of %lu rows, %lu cols over field %lu\n",
                           nor,      noc,                fdef);
        f = malloc(FIELDLEN);
        FieldSet(fdef,f);
        f8=(uint8_t *)f;
printf("    Characteristic %lu, degree %lu\n",
                        f->charc,    f->pow);
...
printf("    paktyp=%d ppaktyp=%d spaclev=%d\n",f->paktyp, f->ppaktyp, f->spaclev);
printf(
"    addtyp=%d, multyp=%d, madtyp=%d, paddtyp=%d, pmultyp=%d, pmadtyp=%d\n",
  f->addtyp, f->multyp, f->madtyp, f->paddtyp, f->pmultyp, f->pmadtyp);
if(f->madtyp==11)
printf("Big Char 2  clpm-e=%016lx   clpm-p=%016lx    clpm-s=%ld\n",
...
printf("    Linear forms use of cauldron %ld alcove %ld pextype %d pastype %d\n",
                                   f->cauldron, f->alcove, f->pextype, f->pastype );
...
// This check should be done outside DEBUG
        ...
        if( (f->cauldron==0) && (f->linfscheme!=0) )
            printf("Linear functions available on Dfmt base\n");
        DSSet(f,noc,&ds);
...
printf("Type 3 - map/permutation, domain %ld, image %ld\n",
    ...
    if(hdr[0]==4)
    {
printf("Type 4 - polynomial degree %ld with %ld factors over field %ld\n",
                               hdr[2],      hdr[3],               hdr[1]);
    }
    return 0;
}
