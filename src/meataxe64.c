...
// too hard to seed the MT stuff in integer.c and the very nice arc4random
// functions
// don't seem to work on Linux (although I may be being stupid)

static inline UInt random64() {
  GAP_STATIC_ASSERT((UInt)RAND_MAX + 1 >= 1UL << 31,
                    "Random generates too few bits");
...
static Int InitKernel(StructInitInfo *module) {
  ...
  //  ImportFuncFromLibrary("Display", &DisplayOp);
