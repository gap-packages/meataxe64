...
int bytedisp(DSPACE * ds, uint64_t *cols, uint64_t nocols, 
              uint64_t *bdisp, int * first, int * len)
{
    const FIELD * f;
    f=ds->f;
    if(ds->ground==0)
...
int trybyte(uint64_t * cols,uint64_t nocol,uint32_t * mex,
            uint64_t entbyte, uint64_t fdef)
{
    uint64_t bytes,i,bwant;
    bytes=nocol/entbyte;
    if( (bytes*entbyte)!=nocol ) return 0;
    for(i=0;i<bytes;i++)
    {
        bwant=cols[entbyte*i]/entbyte;
        if( (bwant*entbyte)!=cols[entbyte*i] ) return 0;
        mex[2+i]=bwant;
        if( cols[entbyte*(i+1)-1]!=((bwant+1)*entbyte-1) ) return 0;
    }
    mex[0]=bytes;
    mex[1]=fdef;
    return 2+bytes; 
...
int trygen (uint64_t * cols,uint64_t nocol,uint32_t * mex,
            uint64_t entbyte, uint64_t fdef)
{
    uint64_t i,j,bwant,obwant,blocks,multi,M1,lose,divby,powp;
    blocks=0;
    multi=1;
    obwant=1000000000000;
    powp=1;
    lose=0;      // eliminate compiler warnings
    for(i=0;i<nocol;i++)
    {
        bwant=cols[i]/entbyte;
        if( (bwant==obwant) && (cols[i]==(cols[i-1]+1) ) )
        {
            lose--;
            powp*=fdef;
        }
        ...
        obwant=bwant;
        divby=1;
        for(j=0;j<lose;j++) divby = divby*fdef;
        M1=4294967296/divby;
        if((fdef&1)!=0) M1++;
        mex[4*blocks - 3]=M1;
        mex[4*blocks - 2]=bwant;
        mex[4*blocks - 1]=powp;
        mex[4*blocks    ]=multi;
    }
    mex[0]=0x20+blocks;
...
int  MakeMex(DSPACE * ds, uint64_t * cols, uint64_t nocol,
                    uint32_t * mex)
{
    const FIELD * f;
    int paktyp;
    uint64_t fdef,i,entbyte;
    ...
    if(nocol==0)
    {
        ...
        mex[1]=0;
        mex[2]=0;
        return 3;        // needs two spare
    }
    if( (paktyp==0) && (nocol==1) )  // 64 bit
    {
        mex[0]=0x4D;
        mex[1]=0;        // unused
        mex[2]=cols[0];
    ...
    if( (paktyp==1) && (nocol<=3) )  // 32 bit
    {
        mex[0]=0x48+nocol;
        mex[1]=fdef;
        for(i=0;i<nocol;i++) mex[2+i]=cols[i];
        return nocol+2;
    }
    if( (paktyp==2) && (nocol<=7) )  // 16 bit
    {
        mex[0]=0x40+nocol;
        mex[1]=fdef;
        for(i=0;i<nocol;i++) mex[2+i]=cols[i];
    ...
    if(paktyp>3)
    {
// First go - are they all bytes?  If so, mode 01-0F return
        i=trybyte(cols,nocol,mex,entbyte,fdef);
        if(i!=0) return i;
// second go - look for special cases - not yet.
// third go - general case
        i=trygen(cols,nocol,mex,entbyte,fdef);
        if(i!=0) return i;
    }
    printf("mex called with impossible parameters\n");
    exit(19);
}
