/* pcrit1s.s x86 assembler */
// parms rdi rsi rdx rcx r8 r9 
// scratch rax (RV) r10 r11
// not scratch rbx rbp r12 r13 r14 r15

// uint64_t * pcmex uint32_t * program, Dfmt * vector
//            =====
//              %rdi              %rsi

// %rdi   Program pointer
// %rsi   Vector pointer
// %edx   Program mode 0(%rdi)
// %ecx   Main index so get e.g. (%rsi,%rcx,2)
// %r8d   Multiplier 4(%rdi)
// %r9    Next Dfmt fetched
// %r10   Power of multiplier
// %rax   Return value being built

// Version 1 - no special cases 10-1F

// MODE 01-0F get 1-15 bytes scaled by 4(%rdi) little-endian
// 01 5 muop ; 02 11 muop 1 mul ; 03 17 muop 3 mul ; +7 muop +2 mul

	.text
	.globl	pcmex
	.type	pcmex, @function
pcmex:
        movl    0(%rdi),%edx   /* get MODE word          */
        movl    8(%rdi),%ecx   /* get IX1 for all cases  */
        testl   $0x70,%edx     /* Ck if mode in {00-0F}  */
        jnz     pcmex20        /* no - analyze further   */

/* MODE 01-0F - 1-15 complete bytes */

        movzbl  0(%rsi,%rcx),%eax  /* get a[IX1] of Dfmt */
        cmpl    $0x01,%edx     /* stop if MODE=01        */
        jne     pcmex02        /* at least two bytes     */
        ret                    /* MODE=01                */
/* 2+ bytes case */
pcmex02:
        movl    4(%rdi),%r8d    /* get multiplier        */
        movl    12(%rdi),%ecx   /* get IX2               */
        movzbl  0(%rsi,%rcx),%r9d  /* get a[IX2]         */
        imull   %r8d,%r9d       /* mul by M^1 (fits 32)  */
        addl    %r9d,%eax       /* add in - (fits 32)    */
        movq    %r8,%r10        /* Hopefully zero cost   */
        subl    $0x02,%edx      /* stop if MODE=02       */
        jne     pcmex04         /* at least three bytes  */
        ret

/* 3-15 bytes */

pcmex03:
        addq    $4,%rdi         /* up for next index */
pcmex04:
        imulq   %r8,%r10        /* next power of multiplier */
        movl    16(%rdi),%ecx   /* get next index */
        movzbq  0(%rsi,%rcx),%r9  /* get next byte */
        imulq   %r10,%r9        /* scale into position */
        addq    %r9,%rax        /* add another one in */
        subl    $0x01,%edx      /* finished yet?      */
        jne     pcmex03         /* no go round again  */
        ret

pcmex20:
        testl   $0x50,%edx     /* Check MODE in {20-2F}  */
        jnz     pcmex10        /* no - analyze further   */
        movzbq  0(%rsi,%rcx),%rax  /* get a[IX1] of Dfmt */
        imull   4(%rdi),%eax   /* multiply by M1 32 bits */
        movl    12(%rdi),%r8d  /* get M2 to 64 bits      */
        imulq   %r8,%rax       /* multiply 64 by M2      */
        shrq    $32,%rax       /* shift result into low  */
        subl    $0x21,%edx     /* stop if MODE=21        */
        jne     pcmex22        /* at least two bytes     */
        ret                    /* MODE=21                */
pcmex22:
        addq    $16,%rdi       /* increment program ctr  */
        movl    8(%rdi),%ecx   /* get index              */
        movzbl  0(%rsi,%rcx),%r9d  /* get a[IX] of Dfmt  */
        imull   4(%rdi),%r9d   /* multiply by M1 32 bits */
        movl    12(%rdi),%r8d  /* get M2 to 64 bits      */
        imulq   %r8,%r9        /* multiply 64 by M2      */
        shrq    $32,%r9        /* to bottom 32 bits      */
        movl    16(%rdi),%r10d /* get M3 to 64 bits      */
        imulq   %r10,%r9       /* multiply by that       */
        addq    %r9,%rax       /* add in the next chunk  */
        subl    $1,%edx        /* stop if all done       */
        jne     pcmex22        /* more bytes             */
        ret                    /* MODE=22-3F             */

/* MODE = 10 M1 IX   M2   M3  M4                         */
/*    0()   4() 8() 12() 16() 20()                       */
/* a[IX1]*M1(64)>>32+{a[IX1+1]*M2(32)*M3(64)>>32}*M4     */
pcmex10:
        testl  $0x40,%edx
        jnz    pcmex40
/* cmpl $0x10,%edx          if other special cases */
/* jne  pcmex11                                    */
        movzbq  0(%rsi,%rcx),%rax  /* get a[IX1] of Dfmt */
        movl    4(%rdi),%r8d   /* get M1 to 64 bits      */
        imulq   %r8,%rax       /* multiply 64 by M2      */
        shrq    $32,%rax       /* shift result into low  */
        movzbl  1(%rsi,%rcx),%r9d  /* get a[IX+1] of Dfmt*/
        imull   12(%rdi),%r9d  /* multiply by M1 32 bits */
        movl    16(%rdi),%r8d  /* get M2 to 64 bits      */
        imulq   %r8,%r9        /* multiply 64 by M2      */
        shrq    $32,%r9        /* to bottom 32 bits      */
        movl    20(%rdi),%r10d /* get M3 to 64 bits      */
        imulq   %r10,%r9       /* multiply by that       */
        addq    %r9,%rax       /* add in the next chunk  */
        ret                    /* MODE=22-3F             */

/* MODE = 41-47 16-bit, 49-4B 32 bit or 4D 64-bit 4E zero */

pcmex40:
        testl   $0x8,%edx      /* 18-1F at pcmex18       */
        jnz     pcmex48
        movzwl  0(%rsi,%rcx,2),%eax  /* get a uint16 of Dfmt */
        cmpl    $0x41,%edx
        jne     pcmex42        /* no more words to do? */
        ret                    /* MODE=41 */
pcmex42:
        movl    4(%rdi),%r8d    /* get multiplier        */
        movl    12(%rdi),%ecx   /* get IX2               */
        movzwl  0(%rsi,%rcx,2),%r9d  /* get a[IX2]       */
        imull   %r8d,%r9d       /* mul by M^1 (fits 32)  */
        addl    %r9d,%eax       /* add in - (fits 32)    */
        movq    %r8,%r10        /* Hopefully zero cost   */
        cmpl    $0x42,%edx      /* stop if MODE=42       */
        jne     pcmex04         /* at least three 16's   */
        ret
pcmex43:
        addq    $4,%rdi         /* up for next index */
        subl    $1,%edx         /* one fewer to do  */
pcmex44:
        imulq   %r8,%r10        /* next power of multiplier */
        movl    16(%rdi),%ecx   /* get next index */
        movzwq  0(%rsi,%rcx,2),%r9  /* get next word */
        imulq   %r10,%r9        /* scale into position */
        addq    %r9,%rax        /* add another one in */
        cmpl    $0x43,%edx      /* finished yet?      */
        jne     pcmex43         /* no go round again  */
        ret

pcmex48:
        testl   $0x4,%edx      /* 4C    at pcmex1d       */
        jnz     pcmex4D        /* for 64-bit extract     */
        movzwq  0(%rsi,%rcx,4),%rax  /* get a uint32 of Dfmt */
        cmpl    $0x49,%edx
        jne     pcmex4A        /* no more words to do? */
        ret                   
pcmex4A:
        movl    4(%rdi),%r8d    /* get multiplier */
pcmex4B:
        movl    12(%rdi),%ecx   /* get next index */
        movl    0(%rsi,%rcx,4),%r9d  /* get next entry byte */
        imulq   %r8,%r10
        imulq   %r10,%r9
        addq    %r9,%rax        /* add in so two done now */
        addq    $4,%rdi
        subl    $1,%edx
        cmpl    $0x49,%edx
        jne     pcmex4B
